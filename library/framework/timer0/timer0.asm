...
;timer0.c,32 :: 		void TIMER0_init(double tempo_desejado)
;timer0.c,35 :: 		bool *ptr = &timer0.is_finalizado[0];
	MOVLW       _timer0+0
	MOVWF       TIMER0_init_ptr_L0+0 
	MOVLW       hi_addr(_timer0+0)
	MOVWF       TIMER0_init_ptr_L0+1 
;timer0.c,37 :: 		for (i = 0; i < TIMER0_LENGTH; ++i)
...
;timer0.c,38 :: 		*ptr++ = false;
	GOTO        L_TIMER0_init0
L_TIMER0_init1:
;timer0.c,42 :: 		clr_bit(T0CON, TMR0ON);
	BCF         T0CON+0, 7 
;timer0.c,43 :: 		set_bit(T0CON, T08BIT);
	BSF         T0CON+0, 6 
;timer0.c,44 :: 		clr_bit(T0CON, T0CS);
	BCF         T0CON+0, 5 
;timer0.c,45 :: 		clr_bit(T0CON, T0SE);
	BCF         T0CON+0, 4 
;timer0.c,46 :: 		clr_bit(T0CON, PSA);
	BCF         T0CON+0, 3 
;timer0.c,47 :: 		set_bit(T0CON, T0PS2);
	BSF         T0CON+0, 2 
;timer0.c,48 :: 		clr_bit(T0CON, T0PS1);
	BCF         T0CON+0, 1 
...
;timer0.c,55 :: 		void TIMER0_start(bool isStart)
;timer0.c,58 :: 		if (isStart)
	MOVF        FARG_TIMER0_start_isStart+0, 1 
	BTFSC       STATUS+0, 2 
	GOTO        L_TIMER0_start3
;timer0.c,60 :: 		set_bit(INTCON, TMR0IE);
	BSF         INTCON+0, 5 
;timer0.c,61 :: 		set_bit(T0CON, TMR0ON);
	BSF         T0CON+0, 7 
;timer0.c,62 :: 		}
	GOTO        L_TIMER0_start4
L_TIMER0_start3:
;timer0.c,65 :: 		clr_bit(INTCON, TMR0IE);
	BCF         INTCON+0, 5 
;timer0.c,66 :: 		clr_bit(T0CON, TMR0ON);
	BCF         T0CON+0, 7 
;timer0.c,67 :: 		}
L_TIMER0_start4:
;timer0.c,68 :: 		}
...
;timer0.c,70 :: 		void TIMER0_ISR()
;timer0.c,75 :: 		for (i = 0, ptr = &tmr0_ticks[0]; i < TIMER0_LENGTH; ++i, ++ptr)
	CLRF        R7 
	CLRF        R8 
...
;timer0.c,77 :: 		*ptr += 1;
	...
	MOVF        R1, 0 
	MOVWF       POSTINC1+0 
;timer0.c,79 :: 		if (*ptr >= TIMER0_IS_FINALIZADO[i])
	MOVFF       R9, FSR0L+0
	MOVFF       R10, FSR0H+0
	MOVF        POSTINC0+0, 0 
	MOVWF       R5 
...
L__TIMER0_ISR14:
	BTFSS       STATUS+0, 0 
	GOTO        L_TIMER0_ISR8
;timer0.c,81 :: 		timer0.is_finalizado[i] = true;
	MOVLW       _timer0+0
	ADDWF       R7, 0 
	MOVWF       FSR1L+0 
	MOVLW       hi_addr(_timer0+0)
