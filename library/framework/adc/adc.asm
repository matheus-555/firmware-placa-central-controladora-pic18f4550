...
;adc.c,22 :: 		void ADC_init()
;adc.c,27 :: 		ADC_QTDE_LEITURA_BINARIO = MATH_get_exponte_base_2((uint16_t) ADC_QTDE_LEITURA);
	MOVLW       16
...
;adc.c,31 :: 		set_bit(TRISA, ADC_PIN_AD_CH_0);
	BSF         TRISA+0, 0 
;adc.c,32 :: 		set_bit(TRISA, ADC_PIN_AD_CH_1);
	BSF         TRISA+0, 1 
;adc.c,33 :: 		set_bit(TRISA, ADC_PIN_AD_CH_2);
	BSF         TRISA+0, 2 
;adc.c,35 :: 		for (i = 0; i < ADC_QTDE_CH; ++i)
...
;adc.c,36 :: 		ADC_variable.an[i] = 0;
	GOTO        L_ADC_init0
L_ADC_init1:
;adc.c,39 :: 		ADCON1 = 0x0C;
	MOVLW       12
	MOVWF       ADCON1+0 
;adc.c,43 :: 		set_bit(ADCON2, ADFM);
	BSF         ADCON2+0, 7 
;adc.c,46 :: 		set_bit(ADCON2, ACQT2);
	BSF         ADCON2+0, 5 
;adc.c,47 :: 		set_bit(ADCON2, ACQT1);
	BSF         ADCON2+0, 4 
;adc.c,48 :: 		clr_bit(ADCON2, ACQT0);
	BCF         ADCON2+0, 3 
;adc.c,51 :: 		clr_bit(ADCON2, ADCS2);
	BCF         ADCON2+0, 2 
;adc.c,52 :: 		clr_bit(ADCON2, ADCS1);
	BCF         ADCON2+0, 1 
;adc.c,53 :: 		set_bit(ADCON2, ADCS0);
	BSF         ADCON2+0, 0 
;adc.c,56 :: 		set_bit(ADCON0, ADON);
	BSF         ADCON0+0, 0 
;adc.c,57 :: 		}
L_end_ADC_init:
	RETURN      0
...
;adc.c,60 :: 		uint16_t ADC_read_channel(uint8_t ch)
;adc.c,63 :: 		ADCON0 |= (ch << 2);
	...
	MOVF        R0, 0 
	IORWF       ADCON0+0, 1 
;adc.c,69 :: 		MOVLW 14                //Carrega o literal 10 em WREG (1 ciclo)
	MOVLW       14
;adc.c,70 :: 		MOVWF __ASM_CICLOS      //Move WREG para o registrador __ASM_CICLOS (1 ciclo)
	MOVWF       32, 1
;adc.c,72 :: 		DELAY_LOOP:
DELAY_LOOP:
;adc.c,73 :: 		NOP                     //No Operation (1 ciclo)
	NOP
;adc.c,74 :: 		DECFSZ __ASM_CICLOS, 1  //Decrementa __ASM_CICLOS e salta se zero (2 ciclos se não saltar, 1 ciclo se saltar)
	DECFSZ      32, 1, 1
;adc.c,75 :: 		GOTO DELAY_LOOP         //Vai para DELAY_LOOP (2 ciclos)
	GOTO        DELAY_LOOP
;adc.c,80 :: 		set_bit(ADCON0, GO_DONE);
	BSF         ADCON0+0, 1 
;adc.c,83 :: 		while (cmp_bit(ADCON0, GO_DONE));
L_ADC_read_channel4:
	BTFSS       ADCON0+0, 1 
	GOTO        L_ADC_read_channel5
	GOTO        L_ADC_read_channel4
L_ADC_read_channel5:
;adc.c,86 :: 		ADCON0 &= 0x03;
	MOVLW       3
	ANDWF       ADCON0+0, 1 
;adc.c,89 :: 		return ((ADRESH << 8) | ADRESL);
	MOVF        ADRESH+0, 0 
	MOVWF       R1 
	CLRF        R0 
	MOVF        ADRESL+0, 0 
...
;adc.c,92 :: 		void ADC_read_all()
;adc.c,103 :: 		for (canal = 0; canal < ADC_QTDE_CH; ++canal)
...
;adc.c,107 :: 		for (i = 0; i < ADC_QTDE_LEITURA; ++i)
...
;adc.c,108 :: 		tmp_val += ADC_read_channel(canal);
...
L_ADC_read_all10:
;adc.c,110 :: 		ADC_variable.an[canal] = (tmp_val >> ADC_QTDE_LEITURA_BINARIO);
	MOVF        ADC_read_all_canal_L0+0, 0 
	MOVWF       R0 
