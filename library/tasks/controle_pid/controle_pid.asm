...
;controle_pid.c,73 :: 		void CONTROLE_PID_init()
;controle_pid.c,75 :: 		isStart = false;
	CLRF        controle_pid_isStart+0 
;controle_pid.c,76 :: 		stop();
...
;controle_pid.c,79 :: 		void CONTROLE_PID_main()
;controle_pid.c,81 :: 		MODO_FUNCIONAMENTO_T = TASK_CONTROLE_PID;
	MOVLW       5
	MOVWF       _writeBuffer+6 
;controle_pid.c,83 :: 		if (!PID_btn_stop)
	BTFSC       PORTB+0, 2 
	GOTO        L_CONTROLE_PID_main0
;controle_pid.c,84 :: 		isStart = false;
	CLRF        controle_pid_isStart+0 
	GOTO        L_CONTROLE_PID_main1
L_CONTROLE_PID_main0:
;controle_pid.c,85 :: 		else if (!PID_btn_start)
	BTFSC       PORTB+0, 0 
	GOTO        L_CONTROLE_PID_main2
;controle_pid.c,87 :: 		isStart = true;
	MOVLW       1
	MOVWF       controle_pid_isStart+0 
;controle_pid.c,88 :: 		start();
	CALL        controle_pid_start+0, 0
;controle_pid.c,89 :: 		}
L_CONTROLE_PID_main2:
L_CONTROLE_PID_main1:
;controle_pid.c,91 :: 		if (isStart)
	MOVF        controle_pid_isStart+0, 1 
	BTFSC       STATUS+0, 2 
	GOTO        L_CONTROLE_PID_main3
;controle_pid.c,93 :: 		if (SOFT_TIMER_delay_ms(&timer[T_PID], 100))
...
;controle_pid.c,95 :: 		tank_level = PID_level_meter;
...
;controle_pid.c,96 :: 		setpoint = PID_setpoint;
...
;controle_pid.c,98 :: 		control_output = calculate_PID(setpoint, tank_level);
	MOVF        R0, 0 
...
;controle_pid.c,100 :: 		if (control_output > 0)
	MOVF        R0, 0 
	MOVWF       R4 
	MOVF        R1, 0 
	MOVWF       R5 
...
;controle_pid.c,103 :: 		PID_discharge_valve(0);
	CLRF        _writeBuffer+20 
	CLRF        _writeBuffer+21 
;controle_pid.c,104 :: 		PWM2_set_duty_cycle(0);
	CLRF        FARG_PWM2_set_duty_cycle_duty_porcent+0 
	CALL        _PWM2_set_duty_cycle+0, 0
;controle_pid.c,107 :: 		PID_fill_valve((unsigned)control_output);
...
;controle_pid.c,108 :: 		value_pwm = (control_output / 1023) * 100;
	MOVLW       0
	MOVWF       R4 
	MOVLW       192
	MOVWF       R5 
...
;controle_pid.c,109 :: 		PWM1_set_duty_cycle(value_pwm > 100 ? 100 : value_pwm);
	MOVLW       0
	MOVWF       R2 
	MOVF        R1, 0 
	SUBWF       R2, 0 
...
;controle_pid.c,113 :: 		control_output = -control_output;
...
;controle_pid.c,116 :: 		PWM1_set_duty_cycle(0);
	...
	CALL        _PWM1_set_duty_cycle+0, 0
;controle_pid.c,119 :: 		PID_discharge_valve((unsigned)-control_output);
	MOVLW       0
...
;controle_pid.c,120 :: 		value_pwm = (-control_output / 1023) * 100;
	MOVLW       0
	XORWF       controle_pid_control_output+0, 0 
	MOVWF       R0 
	MOVLW       0
	...
	MOVF        R1, 0 
	MOVWF       controle_pid_value_pwm+1 
;controle_pid.c,121 :: 		PWM2_set_duty_cycle(value_pwm > 100 ? 100 : value_pwm);
	MOVLW       0
	MOVWF       R2 
	MOVF        R1, 0 
	SUBWF       R2, 0 
...
;controle_pid.c,124 :: 		} // end start
	GOTO        L_CONTROLE_PID_main11
L_CONTROLE_PID_main3:
;controle_pid.c,126 :: 		stop();
	CALL        controle_pid_stop+0, 0
...
;controle_pid.c,129 :: 		static void stop()
;controle_pid.c,132 :: 		PID_discharge_valve(0);
	CLRF        _writeBuffer+20 
	CLRF        _writeBuffer+21 
;controle_pid.c,133 :: 		PWM2_set_duty_cycle(0);
	CLRF        FARG_PWM2_set_duty_cycle_duty_porcent+0 
	CALL        _PWM2_set_duty_cycle+0, 0
;controle_pid.c,135 :: 		PID_fill_valve(0);
	CLRF        _writeBuffer+18 
	CLRF        _writeBuffer+19 
;controle_pid.c,136 :: 		PWM1_set_duty_cycle(0);
	CLRF        FARG_PWM1_set_duty_cycle_duty_porcent+0 
	CALL        _PWM1_set_duty_cycle+0, 0
;controle_pid.c,139 :: 		PID_start_light_off;
	BCF         LATD+0, 0 
;controle_pid.c,142 :: 		if (SOFT_TIMER_delay_ms(&timer[T_TGL_STOP], 500))
...
;controle_pid.c,143 :: 		PID_stop_tgl;
	BTG         LATD+0, 2 
...
;controle_pid.c,146 :: 		static void start()
;controle_pid.c,148 :: 		PID_stop_light_off;
	BCF         LATD+0, 2 
;controle_pid.c,149 :: 		PID_start_light_on;
	BSF         LATD+0, 0 
;controle_pid.c,150 :: 		}
L_end_start:
	RETURN      0
...
;controle_pid.c,152 :: 		static float calculate_PID(float setpoint, float nivel_tanque)
;controle_pid.c,154 :: 		float error = setpoint - nivel_tanque;
...
;controle_pid.c,157 :: 		integral += error;
...
;controle_pid.c,162 :: 		if (integral < -MAX_VAL_INTEGRAL)
	MOVLW       0
	MOVWF       R4 
	MOVLW       0
	MOVWF       R5 
...
;controle_pid.c,163 :: 		integral = -MAX_VAL_INTEGRAL; // Limitar integral para antiwindup
	MOVLW       0
	MOVWF       controle_pid_integral+0 
	MOVLW       0
	MOVWF       controle_pid_integral+1 
...
;controle_pid.c,165 :: 		derivative = (error - previous_error);
	...
	MOVWF       R3 
	CALL        _Sub_32x32_FP+0, 0
;controle_pid.c,168 :: 		derivative = ALPHA * derivative + (1 - ALPHA) * previous_derivative;
	MOVLW       205
	MOVWF       R4 
	MOVLW       204
	MOVWF       R5 
...
;controle_pid.c,170 :: 		previous_derivative = derivative;
...
;controle_pid.c,172 :: 		return (KP * error) + (KI * integral) + (KD * derivative);
	MOVLW       0
	MOVWF       R0 
	MOVLW       0
	MOVWF       R1 
