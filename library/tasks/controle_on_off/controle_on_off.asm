...
;controle_on_off.c,73 :: 		MODO_FUNCIONAMENTO_T = TASK_CONTROLE_ON_OFF;
	MOVLW       6
	MOVWF       _writeBuffer+6 
;controle_on_off.c,74 :: 		isStart = false;
	CLRF        controle_on_off_isStart+0 
;controle_on_off.c,75 :: 		stop();
	CALL        controle_on_off_stop+0, 0
;controle_on_off.c,76 :: 		}
...
;controle_on_off.c,78 :: 		void CONTROLE_ON_OFF_main()
;controle_on_off.c,80 :: 		if (!ON_OFF_btn_stop)
	BTFSC       PORTB+0, 2 
	GOTO        L_CONTROLE_ON_OFF_main1
;controle_on_off.c,81 :: 		isStart = false;
	CLRF        controle_on_off_isStart+0 
	GOTO        L_CONTROLE_ON_OFF_main2
L_CONTROLE_ON_OFF_main1:
;controle_on_off.c,82 :: 		else if (!ON_OFF_btn_start)
	BTFSC       PORTB+0, 0 
	GOTO        L_CONTROLE_ON_OFF_main3
;controle_on_off.c,84 :: 		isStart = true;
	MOVLW       1
	MOVWF       controle_on_off_isStart+0 
;controle_on_off.c,85 :: 		start();
	CALL        controle_on_off_start+0, 0
;controle_on_off.c,86 :: 		}
L_CONTROLE_ON_OFF_main3:
L_CONTROLE_ON_OFF_main2:
;controle_on_off.c,88 :: 		if (isStart)
	MOVF        controle_on_off_isStart+0, 1 
	BTFSC       STATUS+0, 2 
	GOTO        L_CONTROLE_ON_OFF_main4
;controle_on_off.c,90 :: 		if (SOFT_TIMER_delay_ms(&timer[T_ON_OFF], ON_OFF_START_CALC_MS))
...
;controle_on_off.c,92 :: 		if (ON_OFF_setpoint != ON_OFF_level_meter)
...
;controle_on_off.c,94 :: 		if (ON_OFF_level_meter < ON_OFF_setpoint)
	MOVF        _readBuffer+14, 0 
	MOVWF       R1 
	CLRF        R0 
	MOVF        R0, 0 
...
;controle_on_off.c,97 :: 		ON_OFF_discharge_valve(0);
	CLRF        _writeBuffer+20 
	CLRF        _writeBuffer+21 
;controle_on_off.c,98 :: 		PWM2_set_duty_cycle(0);
	CLRF        FARG_PWM2_set_duty_cycle_duty_porcent+0 
	CALL        _PWM2_set_duty_cycle+0, 0
;controle_on_off.c,101 :: 		ON_OFF_fill_valve(1000);
	MOVLW       232
	MOVWF       _writeBuffer+18 
	MOVLW       3
	MOVWF       _writeBuffer+19 
;controle_on_off.c,102 :: 		PWM1_set_duty_cycle(100);
	MOVLW       100
	MOVWF       FARG_PWM1_set_duty_cycle_duty_porcent+0 
	CALL        _PWM1_set_duty_cycle+0, 0
;controle_on_off.c,103 :: 		}
	GOTO        L_CONTROLE_ON_OFF_main8
L_CONTROLE_ON_OFF_main7:
;controle_on_off.c,104 :: 		else if (ON_OFF_level_meter > ON_OFF_setpoint)
	MOVF        _readBuffer+14, 0 
	MOVWF       R1 
	CLRF        R0 
	MOVF        R0, 0 
...
;controle_on_off.c,107 :: 		ON_OFF_fill_valve(0);
	CLRF        _writeBuffer+18 
	CLRF        _writeBuffer+19 
;controle_on_off.c,108 :: 		PWM1_set_duty_cycle(0);
	CLRF        FARG_PWM1_set_duty_cycle_duty_porcent+0 
	CALL        _PWM1_set_duty_cycle+0, 0
;controle_on_off.c,111 :: 		ON_OFF_discharge_valve(1000);
	MOVLW       232
	MOVWF       _writeBuffer+20 
	MOVLW       3
	MOVWF       _writeBuffer+21 
;controle_on_off.c,112 :: 		PWM2_set_duty_cycle(100);
	MOVLW       100
...
;controle_on_off.c,118 :: 		ON_OFF_fill_valve(0);
	CLRF        _writeBuffer+18 
	CLRF        _writeBuffer+19 
;controle_on_off.c,119 :: 		PWM1_set_duty_cycle(0);
	CLRF        FARG_PWM1_set_duty_cycle_duty_porcent+0 
	CALL        _PWM1_set_duty_cycle+0, 0
;controle_on_off.c,122 :: 		ON_OFF_discharge_valve(0);
	CLRF        _writeBuffer+20 
	CLRF        _writeBuffer+21 
;controle_on_off.c,123 :: 		PWM2_set_duty_cycle(0);
	CLRF        FARG_PWM2_set_duty_cycle_duty_porcent+0 
	CALL        _PWM2_set_duty_cycle+0, 0
;controle_on_off.c,124 :: 		}
L_CONTROLE_ON_OFF_main10:
;controle_on_off.c,125 :: 		}
L_CONTROLE_ON_OFF_main5:
;controle_on_off.c,126 :: 		} // end start
	GOTO        L_CONTROLE_ON_OFF_main11
L_CONTROLE_ON_OFF_main4:
;controle_on_off.c,128 :: 		stop();
	CALL        controle_on_off_stop+0, 0
L_CONTROLE_ON_OFF_main11:
;controle_on_off.c,129 :: 		}
